...
function appendAudit(request, record) {
  const authorization = request.headers.authorization;
  fs.appendFileSync(AUDIT_FILE, `${JSON.stringify({
    method: request.method,
    path: request.url,
    accepted: authorization === UPSTREAM_AUTH,
...
const server = http.createServer((request, response) => {
  if (request.headers.authorization !== UPSTREAM_AUTH) {
    ...
    appendAudit(request, { statusCode: 401, requestBytes: 0, responseBytes: 0 });
    return;
  }
  ...
  backend.stdout.on('end', () => {
    ...
    appendAudit(request, { statusCode, requestBytes, responseBytes });
  });
});
