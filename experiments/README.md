...
| [stream-both-directions](./stream-both-directions) | Can a git remote proxy stream both a push (pack in request) and a clone (pack in response) through a fixed, small memory budget — with header auth and front-of-stream ref policy still enforced? | ✅ Yes — proxy holds ~6–18 MB whether the pack is 16 or 256 MB (buffering holds ~2× the pack); **auth needs 0 body bytes, ref policy needs only the pre-pack command section** |
